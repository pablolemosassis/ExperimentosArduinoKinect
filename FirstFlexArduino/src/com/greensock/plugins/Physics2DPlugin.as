...
/**
 * [AS3/AS2 only] Provides simple physics functionality for tweening a DisplayObject's x and y coordinates based on a
 ...
 */
	public class Physics2DPlugin extends TweenPlugin {
		...
		override public function _roundProps(lookup:Object, value:Boolean=true):void {
			if (("physics2D" in lookup) || ("x" in lookup)) {
				_x.r = value;
			}
			if (("physics2D" in lookup) || ("y" in lookup)) {
				_y.r = value;
			}
		}
		...
		override public function setRatio(v:Number):void {
			var time:Number = _tween._time, x:Number, y:Number;
			...
			if (!_skipX) {
				_target.x = (!_x.r) ? x : (x > 0) ? (x + 0.5) >> 0 : (x - 0.5) >> 0;
			}
			if (!_skipY) {
				_target.y = (!_y.r) ? y : (y > 0) ? (y + 0.5) >> 0 : (y - 0.5) >> 0;
			}
